...
- (BOOL)signWithPrivateKeys:(NSArray *)privateKeys andUnspentOuts:(NSArray *) unspentOuts{
    ...
    *keys = [NSMutableArray arrayWithCapacity:privateKeys.count];
    ...
    for (NSUInteger i = 0; i < self.ins.count; i++) {
        ...
        NSMutableData *sig = [NSMutableData data];
        NSData *hash = [self hashForSignatureWitness:i connectedScript:btIn.inScript type:[self getSigHashType] prevValue:preOutValues[i] anyoneCanPay:false];
...
- (NSArray *)unsignedInHashesForBcc:(uint64_t []) preOutValues {
    ...
    for (NSUInteger i = 0; i < self.ins.count; i++) {
        BTIn *btIn = self.ins[i];
        [result addObject:[self hashForSignatureWitness:i connectedScript:btIn.inScript type:[self getSigHashType] prevValue:preOutValues[i] anyoneCanPay:false]];
    }
    return result;
}
