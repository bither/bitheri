...
- (BTTx *)txForAmounts:(NSArray *)amounts andAddress:(NSArray *)addresses dynamicFeeBase:(uint64_t)dynamicFeeBase isNoPrivKey:(BOOL)isNoPrivKey andError:(NSError **)error {
    return [self txForAmounts:amounts andAddress:addresses andChangeAddress:self.address dynamicFeeBase:dynamicFeeBase isNoPrivKey:isNoPrivKey andError:error];
}

- (BTTx *)txForAmounts:(NSArray *)amounts andAddress:(NSArray *)addresses andChangeAddress:(NSString *)changeAddress dynamicFeeBase:(uint64_t)dynamicFeeBase isNoPrivKey:(BOOL)isNoPrivKey andError:(NSError **)error {
    return [self txForAmounts:amounts andAddress:addresses andChangeAddress:changeAddress dynamicFeeBase:dynamicFeeBase isNoPrivKey:isNoPrivKey andError:error coin:BTC];
}

- (BTTx *)txForAmounts:(NSArray *)amounts andAddress:(NSArray *)addresses andChangeAddress:(NSString *)changeAddress dynamicFeeBase:(uint64_t)dynamicFeeBase isNoPrivKey:(BOOL)isNoPrivKey andError:(NSError **)error coin:(Coin)coin {
    BTTx *tx = [[BTTxBuilder instance] buildTxForAddress:self andScriptPubKey:self.scriptPubKey andAmount:amounts andAddress:addresses andChangeAddress:changeAddress dynamicFeeBase:dynamicFeeBase isNoPrivKey:isNoPrivKey andError:error coin:coin];
    if (tx != nil) {
        tx.coin = coin;
    }
    return tx;
