...
#define LogError(frmt, ...)    do{ if(LOG_LEVEL >= 1) NSLog(frmt, ##__VA_ARGS__); } while(0)
#define LogWarn(frmt, ...)     do{ if(LOG_LEVEL >= 2) NSLog(frmt, ##__VA_ARGS__); } while(0)
#define LogInfo(frmt, ...)     do{ if(LOG_LEVEL >= 3) NSLog(frmt, ##__VA_ARGS__); } while(0)
#define LogVerbose(frmt, ...)  do{ if(LOG_LEVEL >= 4) NSLog(frmt, ##__VA_ARGS__); } while(0)

@interface BTCompressingLogFileManager (/* Must be nameless for properties */)
...
@implementation BTCompressingLogFileManager
...
- (void)compressNextLogFile {
    ...
    LogVerbose(@"BTCompressingLogFileManager: compressNextLogFile");
...
- (void)compressionDidSucceed:(DDLogFileInfo *)logFile {
    LogVerbose(@"BTCompressingLogFileManager: compressionDidSucceed: %@", logFile.fileName);
...
- (void)compressionDidFail:(DDLogFileInfo *)logFile {
    LogWarn(@"BTCompressingLogFileManager: compressionDidFail: %@", logFile.fileName);
...
- (void)didArchiveLogFile:(NSString *)logFilePath {
    LogVerbose(@"BTCompressingLogFileManager: didArchiveLogFile: %@", [logFilePath lastPathComponent]);
...
- (void)didRollAndArchiveLogFile:(NSString *)logFilePath {
    LogVerbose(@"BTCompressingLogFileManager: didRollAndArchiveLogFile: %@", [logFilePath lastPathComponent]);
...
- (void)backgroundThread_CompressLogFile:(DDLogFileInfo *)logFile {
    @autoreleasepool {
        ...
        LogInfo(@"BTCompressingLogFileManager: Compressing log file: %@", logFile.fileName);
        ...
        do {
            @autoreleasepool {
                ...
                LogVerbose(@"BTCompressingLogFileManager: Read %li bytes from file", (long) readLength);
                ...
                LogVerbose(@"BTCompressingLogFileManager: Total bytes uncompressed: %lu", (unsigned long) strm.total_in);
                LogVerbose(@"BTCompressingLogFileManager: Total bytes compressed: %lu", (unsigned long) strm.total_out);
                LogVerbose(@"BTCompressingLogFileManager: Compression ratio: %.1f%%",
        ...
        if (error) {
            ...
            LogError(@"Compression of %@ failed: %@", inputFilePath, error);
            error = nil;
            BOOL ok = [[NSFileManager defaultManager] removeItemAtPath:tempOutputFilePath error:&error];
            if (!ok)
                LogError(@"Failed to clean up %@ after failed compression: %@", tempOutputFilePath, error);
            ...
            if (!ok)
                LogWarn(@"Warning: failed to remove original file %@ after compression: %@", inputFilePath, error);
